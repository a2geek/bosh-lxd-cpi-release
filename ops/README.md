...
| Name | Description |
| :--- | :--- |
| [`enable-create-vm-lock.yml`](enable-create-vm-lock.yml) | Enable single-threaded VM deployment. Targeted to remote storage in multi-node clusters. |
...
| [`enable-dhcp-network-assignment.yml`](enable-dhcp-network-assignment.yml) | Enable `dhcp` network assignment. See note in [CONFIGURATION](../docs/CONFIGURATION.md). |
...
| [`enable-predeploy-stemcell.yml`](enable-predeploy-stemcell.yml) | Enable a VM to "pre-deploy" when a stemcell is loaded in order to prime the stemcell image within LXD or Incus. Targeted to remote storage in multi-node clusters. |
