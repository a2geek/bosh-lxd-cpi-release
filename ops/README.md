...
| [`remove-bios-path.yml`](remove-bios-path.yml) | Remove `bios_path` and instead rely on UEFI boot. (Note that Noble stemcells introduce EFI boot and that not all BOSH releases are compatible. YMMV!) | 
| [`set-bios-path.yml`](set-bios-path.yml) | Set `bios_path` to reconfigure the source of the boot BIOS if the default of `bios-256k.bin` needs to be changed. Note that the environment variable `LXD_QEMU_FW_PATH` (where "FW" means firmware) can be setup to point to the BIOS directory. If the LXD snap is used, this appears to be already configured. |
